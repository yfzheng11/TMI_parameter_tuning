...
class MemoryOptimizedReplayBuffer(object):
    def __init__(self, size, frame_history_len, lander=False):
        ...
        of the episode, when there is fewer frames than `frame_history_len`,
        is acceptable.
...
class ReplayBuffer(object):
    def __init__(self, size, ob_dim):
        ...
        self.obs = np.empty((self.size, ob_dim), dtype=np.float32)
        self.next_obs = np.empty((self.size, ob_dim), dtype=np.float32)
